...
exports.insult = {
  ...
  process: (msg, suffix, isEdit, cb) => {
    var randomnumber = Math.floor(Math.random() * (factsData.insult.length - 1 + 1)) + 1;
    cb({
      reply: resolveMention(suffix),
      embed: { description: factsData.insult[randomnumber] }
    }, msg);
  }
}

exports.remaeus_fact = {
  description: 'Blesses you with a fact about our humble leader, Remaeus.',
  process: (msg, suffix, isEdit, cb) => {
    var randomnumber = Math.floor(Math.random() * (factsData.remaeus.length - 1 + 1)) + 1;
    cb({
      embed: {
        color: Doorman.Config.discord.defaultEmbedColor,
        title: 'Remaeus Fact',
        description: factsData.remaeus[randomnumber]
      }
    }, msg);
  }
}

exports.interstella_fact = {
  description: 'Gives you a fact about the movie Interstella 5555',
  process: (msg, suffix, isEdit, cb) => {
    var randomnumber = Math.floor(Math.random() * (factsData.interstella.length - 1 + 1)) + 1;
    cb({
      embed: {
        color: Doorman.Config.discord.defaultEmbedColor,
        title: 'Interstella 5555 Fact',
        description: factsData.interstella[randomnumber]
      }
    }, msg);
  }
}

exports.topology_fact = {
  description: 'Gives you a fact about topology... nerd.',
  process: (msg, suffix, isEdit, cb) => {
    var randomnumber = Math.floor(Math.random() * (factsData.topology.length - 1 + 1)) + 1;
    cb({
      embed: {
        color: Doorman.Config.discord.defaultEmbedColor,
        title: 'Topology Fact',
        description: factsData.topology[randomnumber]
      }
    }, msg);
  }
}
