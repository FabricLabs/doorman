...
function Doorman (config) {
  ...
  self.fabric = new Fabric(self.config.fabric);
...
// TODO: use ES6 inheritance from Scribe (!)
util.inherits(Doorman, require('events').EventEmitter);

Doorman.prototype.stop = async function stop () {
  let self = this;

  self.scribe.log('Stopping...');

  if (self.services) {
    for (let name in self.services) {
      await self.services[name].disconnect();
    }
  }
 ...
 */
Doorman.prototype.start = async function configure () {
  let self = this;

  self.scribe.on('info', self.emit.bind(self));

  self.enable('local');

  if (self.config.services && Array.isArray(self.config.services)) {
    for (let name in self.config.services) {
      self.enable(self.config.services[name]);
    }
  }
