...
class Service extends Scribe {
  ...
  async connect (notify) {
    ...
    try {
      ...
      console.warn('[DOORMAN:SERVICE]', 'Could not restore state:', E);
    }
...
Service.prototype.error = function errorHandler (error) {
  console.error('[DEPRECATED:SEE:SCRIBE]', '[SERVICE:ERROR]', error);
};

Service.prototype.warn = function warningHandler (msg) {
  console.warn('[DEPRECATED:SEE:SCRIBE]', '[SERVICE:WARNING]', msg);
};

Service.prototype.info = function infoHandler (msg) {
  console.log('[DEPRECATED:SEE:SCRIBE]', '[SERVICE:INFO]', msg);
};
