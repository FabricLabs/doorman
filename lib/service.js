...
class Service extends Scribe {
  constructor (config) {
    ...
    // Start with an empty state and no persistence
    this.state = {};
    this.store = null;
...
Service.prototype.connect = async function initialize () {
  // TODO: implement a basic Stream
  this.status = 'connecting';

  this.store = new Fabric.Store({ path: this.config.store });
  ...
  try {
    ...
    console.error('[DOORMAN:SERVICE]', 'Could not restore state:', E);
  }
...
Service.prototype.whisper = async function whisper (target, message) {
  console.log('The "whisper" function is not yet implemented.');
  return this;
}

Service.prototype._getUser = function getUser (id) {
  if (!id) return console.error('Parameter "id" is required.');
...
Service.prototype.commit = async function commit (id) {
  ...
  try {
    ...
    console.error('[DOORMAN:SERVICE]', 'Error saving state:', self.state);
